...
async function fetchTopAnime() {
    try {
        ...
        if (data && data.data) {
            const topAnime = data.data; // Array of top anime
            const popularAnimes = topAnime.slice(0, 10); // Get top 10
            const incomingAnimes = topAnime.filter(anime => anime.airing_start && new Date(anime.airing_start) > new Date());
