...
async function fetchTopAnime() {
    try {
        ...
        if (data && data.data) {
            const topAiring = data.data; // Array of top airing anime
            const popularAnimes = topAiring.slice(0, 10); // Get top 10
            const incomingAnimes = topAiring.filter(anime => anime.airing_start && new Date(anime.airing_start) > new Date());
